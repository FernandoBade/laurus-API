...
export const criarUsuarioSchema = z.object({
    ...
    dataNascimento: z.string().optional().refine(data => !data || (new Date(data).toString() !== 'Invalid Date'), {
    ...
    aparencia: z.enum([Aparencia.DARK, Aparencia.LIGHT], {
        message: `A aparência deve ser 'dark' ou 'light'`
    }),
    idioma: z.enum([Idioma.PT_BR, Idioma.EN_US, Idioma.ES_ES], {
        message: `O idioma deve ser 'pt-BR', 'en-US' ou 'es-ES'`
    }),
    moeda: z.enum([Moeda.BRL, Moeda.USD, Moeda.EUR, Moeda.ARS], {
        message: `A moeda deve ser 'BRL', 'USD', 'EUR' ou 'ARS'`
    }),
    formatoData: z.enum([FormatoData.DD_MM_AAAA, FormatoData.MM_DD_YYYY], {
...
export const atualizarUsuarioSchema = z.object({
    ...
    dataNascimento: z.string().optional().refine(data => !data || (data.length === 24 && new Date(data).toISOString() === data), {
